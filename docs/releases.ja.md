...
- **アプリの店。** ランチャーの先頭に並びます。公開されている一覧を WiFi で取ってきて、
  この機械に載るものを示し、`/app/usr` に入れます。実機 2 種とブラウザのどれでも動きます。
- **機械が勝手にやること (Modern)。** 小さな常駐サービスが動きます。時計はネットワークから
  自分を合わせ、機械は自分のアドレスを知らせ、頼めば喋ります。一覧は書き換えられるファイル
...
- Studio はページ側で配色を選ばなくなりました。機械の中の Config でテーマを選びます。
- Modern のアプリ用プールは 1024 KB、`large_memory = 1` で 2048 KB になりました。Retro は
  変わりません。→ [制約事項](limitations.md#ヒープのサイズ)
- `app_spawn_margin_kb` で、アプリを起動するときに機械の側へ残す内蔵 RAM を決められます。
...
通信仕様は 2.0.0 と同じ 4 なので、2.0.0 の機械には片方のチップだけの書き込みで足ります。
