...
`path` は core 側 (内蔵 flash) のパスです。グラフィックス側ではないことに注意してください。
読める形式は 2 つあり、先頭 4 バイトでどちらかを判別します。

| 形式 | |
|---|---|
| `.map.bin` | 詰めた形。1 タイル 1 バイトで、解析せずそのまま読みます |
| `.json` | 下記の `fmrb_map` v1 JSON |

端末では詰めた形を使ってください。地図の読み込みは以前 JSON の解析で、大きな地図では
1 分近くかかっていました。詰めた形は解析ではなく読み取りなので、同じ地図が数十ミリ秒で
出てきます。JSON は人が読めて差分も取れるので、リポジトリに置くのはこちらにして、
詰めた形を隣に書き出すとよいでしょう。
...
- マップエディタ: タイル配置 + イベント編集 → 詰めた `.map.bin` か `fmrb_map` JSON で書き出し。読み込みは両方に対応
