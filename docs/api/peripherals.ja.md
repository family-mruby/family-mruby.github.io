...
![実機から駆動している 8x8 の WS2812B マトリクスと、エディタで開いたそのソース](../images/photo_led_matrix.jpg)

このアプリはパネルに送るのと同じ模様を画面にも描くので、何もつないでいなくても
動きが分かります。ソースの先頭近くにある `LED_PIN` の `FmrbConst::BOARD` による
分岐にも注目してください。GROVE のピンは機種で違います。
