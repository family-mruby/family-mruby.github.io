...
`P5` は [Processing](https://processing.org/) / [p5.js](https://p5js.org/) 風の描画 DSL です。`FmrbGfx` を内部でラップしており、`fill` / `stroke` / `translate` / `rotate` などの状態を持った p5 スタイルで図形を書けます。`picoruby-fmrb-app` に組み込まれているため、追加の `require` は不要です。
...
| `quad(x0, y0, x1, y1, x2, y2, x3, y3)` | 四角形 (4 点指定) |
| `begin_shape` / `vertex(x, y)` / `end_shape(close = false)` | 自由な多角形。開いて、点を足して、閉じます。`end_shape(true)` で最後の点と最初の点をつなぎます |

!!! note "`alpha` は何もしません"
    p5.js 向けに書いたコードがそのまま動くように受け付けるだけです。この機械にアルファ
    チャンネルはなく、色は 256 色のどれかで不透明です。半透明が要るときは
    [`blend_rect`](fmrb_gfx.md#基本図形) か下の `blend_mode` を使ってください。
...
| `get_pixel(x, y)` | 読み戻し（グラフィックス側まで往復するため遅い） |

!!! warning "`get_pixel` のコスト"
    `get_pixel` は同期呼び出しです。未送信の描画コマンドをすべて送り出し、グラフィックス側で読み出して、答えが返るまで待ちます。Retro ではその答えが UART を往復するので 1 ピクセルに数ミリ秒かかります。Modern はもっと軽いとはいえ往復には変わりありません。ループで多数読むのは避けてください（ピクセル単位の参照より、自前で持つ二次元配列に描画状態を保持する方が高速）。
...
PicoRuby デモ (`/app/demo/picoruby.app.rb`) の P5 のページに、基本図形・アフィン変換・ベジエ・テキスト・円弧・blend・`get_pixel` が一通り入っています。
