...
| `quad(x0, y0, x1, y1, x2, y2, x3, y3)` | Quadrilateral |
| `begin_shape` / `vertex(x, y)` / `end_shape(close = false)` | A free polygon: open the shape, add its points, close it. `end_shape(true)` joins the last point to the first |

!!! note "`alpha` does nothing"
    It is accepted so that code written against p5.js still runs, but this machine has no
    alpha channel — a colour is one of 256, opaque. For translucency use
    [`blend_rect`](fmrb_gfx.md#basic-shapes) or the `blend_mode` below.
...
| `font_height` | Height of the current font, in pixels |
| `text(str, x, y)` | Draw text |

For Japanese fonts, see [FmrbGfx > Japanese Text](fmrb_gfx.md#japanese-text-and-font-switching).
...
| `get_pixel(x, y)` | Read back a pixel (slow due to round-trip to graphics side) |

!!! warning "`get_pixel` cost"
    `get_pixel` is a synchronous call: it flushes every unsent drawing command, reads on the graphics side, and waits for the answer. On Retro that answer crosses the UART link, so a single pixel costs several milliseconds; it is cheaper on Modern but still a round trip. Avoid calling it in loops. Maintaining a local 2D array of drawing state is much faster than per-pixel reads.
...
The P5 pages of the PicoRuby demo (`/app/demo/picoruby.app.rb`) walk through the lot: basic shapes, affine transforms, Bezier, text, arcs, blend and `get_pixel`.
