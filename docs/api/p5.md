...
`P5` is a [Processing](https://processing.org/) / [p5.js](https://p5js.org/)-style drawing DSL. It wraps `FmrbGfx` internally and lets you draw shapes in a p5 style with stateful `fill` / `stroke` / `translate` / `rotate` and more.

!!! note "Library format"
    P5 is not built into the firmware. It is a loadable library located at `flash/lib/p5.rb`, and only apps that use it import it with `require "p5"`. The footprint only applies to apps that use P5.
...
A sample that switches between features (basic shapes / affine transforms / Bezier / text / arcs / blend / get_pixel) is available in `flash/app/demo/p5_test.app.rb`.
