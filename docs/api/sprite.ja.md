...
| `GfxBlock` | 描画コマンド列を グラフィックス側にバイトコードとしてキャッシュ し、可変パラメータだけを再送信する仕組み |
...
| `load_bmp(path)` | BMP ファイルを画像へ読み込み（グラフィックス側でデコード、高速） |
| `destroy` | リソース解放 |
...
画像も配置も、アプリごとではなく本体のものです。どのアプリも同じ枠を分け合います。
...
2. 描画コマンド列を グラフィックス側にコンパイル・キャッシュ
3. `draw(kwargs)` 呼び出しでは 変化したレジスタの値だけを送信して再実行
