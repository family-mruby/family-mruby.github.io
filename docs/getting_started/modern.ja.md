...
- **USB キーボード**を Tab5 の USB-A 端子に挿す。マウスも使いたければ USB ハブを足します。
  動作確認済みの機器は [動作確認済み機器](../compatibility.md) を参照してください
- **M5Stack Tab5 Keyboard** を本体に取り付ける。USB 端子を使いません

マウスは必須ではありません。画面のタッチがそのままトラックパッドとして使えます
([タッチで操作する](#_6) 参照)。ただし窓を動かすような操作は実際のマウスのほうが楽です。
...
左上の **Family mruby** を押すとシステムメニューが出ます。

| 項目 | 内容 |
|---|---|
| Launcher | 入っているアプリの一覧。ここからアプリを起動します |
| File Manager | フラッシュの中のファイルを見る |
| Log Viewer | システムログ |
| Monitor | 動いているタスクとメモリ |
| Set Clock | 日付と時刻の設定 |
| Config | 言語、キーボード配列、カーソル速度、配色、時間帯、WiFi/BLE の自動起動 |
| Storage | キャッシュの削除 |
| Network | WiFi の状態、割り当てられたアドレス、ホスト名 |
| About | バージョンとチップの情報 |
| Reset | 再起動 |

メニューバーの右端には、内蔵メモリの空き、BLE の状態、WiFi の状態、時計が出ます。

### アプリを起動する

**Launcher** を開いて、アイコンをダブルクリックします。アプリを追加した後は、ランチャーの
窓の中で右クリックするとファイルを読み直します。
...
|---|---|
| `Ctrl` + `Q` | 手前のアプリを終了する |
| `Ctrl` + `Tab` | 動いているアプリの間で操作対象を切り替える |

デスクトップ上で — アプリを操作しておらず、ダイアログも出ていないとき — 1 文字打つと
アプリが起動します。
...
この割り当ては `/etc/system_conf.toml` の `[[shortcuts]]` にあり、書き換えられます。
