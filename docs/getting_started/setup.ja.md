...
- Shell（シェル）
- Editor（エディタ）
- ユーザーアプリ（`/flash/app/` 以下にインストール済みのもの）
...
ERROR LED が点いたままの場合は、USB を一度抜いて再起動してみてください。それでも改善しない場合は [BLE ファイルマネージャ](ble_file_manager.md) でログを確認できます。

## 次のステップ

- 自分のアプリを書いてみる → [Hello World](hello_world.md)
- PC からアプリやファイルを転送する → [BLE ファイルマネージャ](ble_file_manager.md)
