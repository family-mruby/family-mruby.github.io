...
| キー | 動作 |
|---|---|
| `Ctrl` + `Q` | 手前のアプリを終了する。全画面のものからも抜けられます |
| `Ctrl` + `Tab` | 動いているアプリの間で操作対象を切り替える |

デスクトップ上で — アプリを操作しておらず、ダイアログも出ていないとき — 1 文字打つと
アプリが起動します。`L` ランチャー、`S` シェル、`E` エディタ、`N` NSF 再生、
`I` HID Inspector。この割り当ては `/etc/system_conf.toml` の `[[shortcuts]]` にあります。

ランチャーでは、アプリを足した後に**右クリック**すると読み直します。一覧はそれ以外では
デスクトップの起動時に一度だけ作られます。
