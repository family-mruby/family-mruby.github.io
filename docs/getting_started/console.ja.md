...
PC やスマートフォンから Bluetooth Low Energy (BLE) 経由で基板内のファイルを読み書きするツールです。`/...` 以下のファイルを Web ブラウザから操作できます。
...
### 方法 1: GitHub Pages 公開版（推奨）

ブラウザで以下を開くだけで利用できます。

[https://family-mruby.github.io/console/](/console/)

GitHub Pages は HTTPS で配信されるため、Web Bluetooth の要件も満たします。

### 方法 2: ローカルで起動（リポジトリに同梱）
...
!!! note "Web Bluetooth は localhost か HTTPS 必須"
    Web Bluetooth API はセキュリティ上の制約で `localhost` または HTTPS でしか動きません。`web_server.rb` が `localhost` で起動するため問題ありません。GitHub Pages 版（方法 1）は HTTPS 配信なのでそのまま動作します。
